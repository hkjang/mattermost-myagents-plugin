...
export function isLangflowStreamingPost(post?: Post | null): post is Post {
    ...
    return props.langflow_stream === 'true' || props.langflow_streaming === 'true' ||
        props.myagents_streaming === 'true' || props.myagents_stream_status === 'streaming';
}
...
export function buildStreamingPostUpdate(state: GlobalState, data?: StreamingPostUpdateEventData): Post | null {
    ...
    return {
        ...
        props: {
            ...existingPost.props,
            myagents_streaming: data?.control === 'end' ? 'false' : 'true',
            myagents_stream_status: data?.control === 'end' ? 'completed' : 'streaming',
            myagents_stream_placeholder: 'false',
        },
    };
}
